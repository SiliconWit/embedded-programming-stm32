...
heroImage: "https://cdn.siliconwit.com/education/embedded-programming-stm32/low-power-production-firmware.png"
---
...
void cmd_dump_log(void) {
    ...
    /* Determine how many bytes to read, handling wrap-around.
     * If write_ptr >= FLASH_LOG_START and the log has not wrapped,
     * we read from start to write_ptr. If write_ptr has wrapped
     * (written past the end and back to the beginning), we read
     * from write_ptr to the end, then from the start to write_ptr. */
    uint32_t total_bytes;
    if (write_ptr >= FLASH_LOG_START) {
        total_bytes = write_ptr - FLASH_LOG_START;
    } else {
        total_bytes = (FLASH_LOG_END + 1 - FLASH_LOG_START);
    }
    uint32_t entries_to_read = total_bytes / ENTRY_SIZE;
    ...
    for (uint32_t i = 0; i < entries_to_read; i++) {
        ...
        if (entry.timestamp == 0xFFFFFFFF) {
            addr += ENTRY_SIZE;
            if (addr >= FLASH_LOG_END) addr = FLASH_LOG_START;
            continue;
        }
...
void peripherals_off(void) {
    /* Set all unused GPIO pins to analog mode FIRST (lowest leakage).
     * This must happen while GPIO clocks are still enabled,
     * otherwise the register writes have no effect. */
    ...
    /* Now disable all peripheral clocks except RTC and backup domain */
    RCC->APB1ENR = RCC_APB1ENR_PWREN | RCC_APB1ENR_BKPEN;
    RCC->APB2ENR = 0;
    RCC->AHBENR = 0;
}
