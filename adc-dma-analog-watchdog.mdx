...
heroImage: "https://cdn.siliconwit.com/education/embedded-programming-stm32/adc-dma-analog-watchdog.png"
---
...
void i2c1_write_reg16(uint8_t addr, uint8_t reg, uint16_t val)
{
...
uint16_t i2c1_read_reg16(uint8_t addr, uint8_t reg)
{
    uint8_t buf[2];
    i2c1_read_burst(addr, reg, buf, 2);
...
:::tip[Instrument it]
The analog watchdog you set up here is a hardware condition-monitor: it watches an ADC channel and fires an interrupt the moment a reading leaves a safe band, with zero CPU polling. Wire an accelerometer or the INA219's current output into it and you have a sensor that raises a flag the instant vibration or current crosses a limit, ideal for a motor or pump. The [STM32](/education/sensor-actuator-interfacing-stm32/) can act locally, cutting a relay, and [raise the alarm](https://siliconwit.io) so a human sees it too.
:::
